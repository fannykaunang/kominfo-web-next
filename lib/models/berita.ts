import { query, queryOne, execute, generateUUID, buildWhereClause, buildPagination } from './db-helpers'
...
export class BeritaRepository {
  ...
  static async findAll(options: {
    ...
    const { page = 1, limit = 10, kategori_id, is_published, is_highlight, search } = options
   ...
   */
  static async getRelated(kategori_id: string, excludeId: string, limit: number = 3): Promise<Berita[]> {
    const sql = `
      SELECT 
        b.*,
        k.nama as kategori_nama,
