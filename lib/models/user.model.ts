...
export async function getAllUsers(
  ...
  const dataQuery = `
    ...
    LIMIT ? OFFSET ?
  `
  const users = await query<SafeUser>(dataQuery, [...params, limit, offset])
...
export async function updateUser(
  ...
  await createLogWithData({
    ...
    data_sesudah: { ...existingUser, ...data, password: data.password ? "[HIDDEN]" : undefined },
  })
}
...
export async function toggleUserActive(
  ...
  await createLogWithData({
    ...
    aksi: "UPDATE",
    modul: "USERS",
    detail_aksi: `${newStatus === 1 ? "Mengaktifkan" : "Menonaktifkan"} user: ${user.name}`,
...
export async function verifyUserEmail(
  ...
  const sql = "UPDATE users SET email_verified = 1, updated_at = NOW() WHERE id = ?"
  await execute(sql, [id])
